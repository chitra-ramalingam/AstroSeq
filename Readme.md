...
This repo is for learning, experimentation, and practical exoplanet ML.
...
- [Data Sources & Time Systems](#data-sources--time-systems)
- [Label Construction (Segment-Level)](#label-construction-segment-level)
- [Model: 1D Inception-ResNet Style CNN](#model-1d-inception-resnet-style-cnn)
- [Training & Metrics](#training--metrics)
- [Starwise 1D CNN: Star-Level Scoring](#starwise-1d-cnn-star-level-scoring)
- [Isolation Forest Attempt](#isolation-forest-attempt)
- [Known Issues / Gotchas](#known-issues--gotchas)
- [Mission-wise classifiers (window=1024, no capping)](#mission-wise-classifiers-window1024-no-capping)
...
Ephemeris-based labeling

For each segment, check if its time span overlaps a predicted transit window (using midtime, period, duration)

Assign segment label:

1 if overlaps a predicted transit
...
(Earlier versions used per_star_cap to cap segments per star to avoid domination; see mission notes below)
...
Conv + BatchNorm with 64 filters, kernel size 7 → captures ingress/bottom/egress-like shapes
...
Second stack after raising channel depth (e.g. to 96)
...
Metrics: ROC-AUC (roc_auc), PR-AUC (pr_auc), binary accuracy
NNet Intuition

Channels: per-timestep features (flux variants)

64 filters early: enough to learn diverse transit/noise shapes without exploding size

Kernel size 7: wide enough to see dip shape, not just single spikes
...
Interpretation: PR-AUC above baseline → ranking is useful

Issue: conservative threshold predicted positive rate ≈ 20% vs true positive ≈ 48%

Segment recall ~0.33 → many positives below chosen threshold

Epoch 40

Overfitting: train loss ↓ while val loss ↑
...
Increasing max epochs further has limited benefit without changing regularization/architecture
...
Takes a limited number of positive and negative stars

(Earlier versions) capped segments per star

Trains the segment model efficiently

Starwise1DCnn (inference / survey stage)

Reuses the trained model (e.g. best_ref.keras)

For each star:

Fetch full light curve (possibly multiple sectors/quarters)

Normalize + segment all available data

Predict per-segment probabilities

Aggregate to a single star-level score (e.g. top-k mean, max, etc.)
...
Training sees a small capped subset; Starwise runs across all segments for all stars

Most wall-time is dominated by Lightkurve/MAST I/O, not the CNN

## Caching:

Light curves cached as .npz

Subsequent runs reuse local data

Cache can grow to tens of GB for large surveys

## Isolation Forest Attempt

Earlier attempt used Isolation Forest for anomaly detection:

Precision/recall tradeoffs were poor

Performance hovered around:
...
Some light curves are empty/too short/fail during download

Recorded and cached as empty to avoid repeated fetch attempts

Download time: fetching thousands of stars is slow → caching is essential
...
star_id, score_raw, p_highclass

This is a model-selected set of strongly transit-like stars, containing a mix of confirmed planets, candidates, and likely false positives

Mission-wise classifiers (window=1024, no capping)

Status: trained mission-specific models for Kepler / K2 / TESS with window length = 1024 and no per-star segment capping.

## TESS runs (sampling experiments)
...
50K cap, 1:1 (balanced, random negs; “Good: 50K cap final”)	—	—	0.6517417	marked good
Epoch 1 snapshot, 1:1 (full ~221k balanced)	0.7198	0.5369	—	marked good
