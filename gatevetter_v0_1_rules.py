...
OUT_PREDICTIONS = ROOT / "gatevetter_v0_1_manual64_predictions.csv"
OUT_CONFUSION = ROOT / "gatevetter_v0_1_confusion_matrix.csv"
OUT_FAILURE_REPORT = ROOT / "gatevetter_v0_1_failure_report.txt"
OUT_RULE_TRACE = ROOT / "gatevetter_v0_1_rule_trace.csv"
OUT_STAGE_G_QUEUE = ROOT / "gatevetter_v0_1_stage_g_review_queue.csv"
...
def no_secondary_present(secondary_ratio: float, secondary_snr: float) -> bool:
    ratio_ok = not finite(secondary_ratio) or secondary_ratio < 0.20
    snr_ok = not finite(secondary_snr) or secondary_snr < 5.0
    return ratio_ok and snr_ok


def duration_not_impossible(duration: float) -> bool:
    return not finite(duration) or duration < 0.25


def high_oot_caveated_candidate_evidence(
    row: pd.Series,
    cnn: float,
    snr: float,
    odd: float,
    ...
    return (
        ...
        and no_secondary_present(secondary_ratio, secondary_snr)
        and duration_not_impossible(duration)
        and not alias_is_high(row.get("alias_risk"))
    )


def huge_fallback_caveated_candidate_evidence(
    row: pd.Series,
    cnn: float,
    snr: float,
    odd: float,
    ...
    return (
        truthy(row.get("fallback_period_flag"))
        and period_count >= 1000
        and cnn >= 0.95
        and finite(snr)
        and 4.0 <= snr <= 25.0
        and finite(odd)
...
def huge_fallback_hard_reject_context(row: pd.Series, odd: float, oot: float, duration: float) -> bool:
    return (
        (finite(odd) and odd <= 0.75)
        or (finite(oot) and oot >= 0.50)
        or alias_is_high(row.get("alias_risk"))
        or (finite(duration) and duration >= 0.25)
    )
...
def gatevet(row: pd.Series) -> dict[str, Any]:
    ...
    if fallback and period_count >= 1000:
        hold_gates.append("gate2_fallback_period_with_ge_1000_candidates")
        trace.append("Gate 2 hold/review: fallback period with candidate period count >= 1000")
        blocked_candidate = True
    elif fallback and period_count >= 500:
    ...
    if blocked_candidate:
        if high_oot_caveated_candidate_evidence(row, cnn, snr, odd, secondary_ratio, secondary_snr, oot, period_count, duration):
            trace.append(
                "Gate 6 caveated review: high OOT/duration caveat but odd-even is strong, "
                "SNR is credible, CNN morphology is high, and no secondary is present"
            )
            components["caveated_high_oot_support"] = 0.16
            candidate_score = max(candidate_score + 0.16, 0.66)
            return finalize_prediction(
                row,
                "caveated_candidate_stage_g_review",
                "caveated_candidate_high_oot_or_duration",
                candidate_score,
                hard_gates,
                hold_gates,
                risk_gates,
        ...
        if huge_fallback_caveated_candidate_evidence(row, cnn, snr, odd, secondary_ratio, secondary_snr, oot, period_count, duration):
            trace.append(
                "Gate 6 caveated review: huge fallback-period clutter but morphology, odd-even, "
                "OOT, SNR, alias, and secondary checks are candidate-like"
            )
            components["caveated_huge_fallback_support"] = 0.18
            candidate_score = max(candidate_score + 0.18, 0.66)
            return finalize_prediction(
                row,
                "caveated_candidate_stage_g_review",
                "caveated_candidate_huge_fallback_period_count",
                candidate_score,
                hard_gates,
                hold_gates,
                risk_gates,
        ...
        if fallback and period_count >= 1000 and huge_fallback_hard_reject_context(row, odd, oot, duration):
            hard_gates.append("gate2_fallback_huge_count_with_failed_context")
            trace.append("Gate 2 hard reject: huge fallback clutter with failed odd-even/OOT/alias/duration context")
            return finalize_prediction(
                row,
                "negative_noise_or_artifact",
                "fallback_period_clutter_failed_context",
                0.0,
                hard_gates,
                hold_gates,
                risk_gates,
...
def finalize_prediction(
    ...
    if prediction == "caveated_candidate_stage_g_review":
        primary_gate = "gate6_caveated_candidate_stage_g_review"
    elif hard_gates:
    ...
    if prediction in {"candidate_like_positive", "caveated_candidate_stage_g_review"}:
    ...
    return {
        ...
        "stage_g_action": (
            "gatevetter_stage_g_review_candidate"
            if prediction in {"candidate_like_positive", "caveated_candidate_stage_g_review"}
            else "no_gatevetter_stage_g_action"
        ),
...
def maybe_autovet_comparison(scored: pd.DataFrame) -> list[str]:
    ...
    return [
        ...
        f"- GateVetter v0.1 candidate-like promotions: {len(promoted_now)}",
...
def failure_report(scored: pd.DataFrame, matrix: pd.DataFrame, stage_g_queue: pd.DataFrame) -> str:
    ...
    stage_g_predictions = scored[
        scored["gatevetter_v0_prediction"].isin({"candidate_like_positive", "caveated_candidate_stage_g_review"})
    ].copy()
    stage_g_recovered = int(stage_g_predictions["manual_truth_class"].eq("candidate_like_positive").sum())
    stage_g_recall = stage_g_recovered / len(positive_truth) if len(positive_truth) else 0.0
    false_stage_g_promotions = stage_g_predictions[
        ~stage_g_predictions["manual_truth_class"].eq("candidate_like_positive")
    ].copy()
    ...
    lines = [
        "GateVetter v0.1 manual-64 calibration report",
        ...
        f"- manual candidates recovered as candidate or caveated Stage G review: {stage_g_recovered} / {len(positive_truth)} = {stage_g_recall:.3f}",
        ...
        f"- false Stage G review promotions: {len(false_stage_g_promotions)}",
        "",
        "Truth counts",
    ]
    ...
    lines.extend(["", "False Stage G review promotions"])
    if false_stage_g_promotions.empty:
        lines.append("- none")
    else:
        for _, row in false_stage_g_promotions.sort_values("gatevetter_score", ascending=False).iterrows():
            lines.append(
                f"- {row['epic_id']}: truth={row['manual_truth_class']}; pred={row['gatevetter_prediction']}; "
                f"score={row['gatevetter_score']}; primary_gate={row['primary_gate']}; trace={row['rule_trace']}"
            )

    lines.extend(["", "Top 20 Stage G review queue"])
    lines.append("- Queue contains GateVetter validated survivors only: candidate_like_positive or caveated_candidate_stage_g_review.")
    lines.append(f"- queued rows: {len(stage_g_queue)}")
    if len(stage_g_queue):
        for _, row in stage_g_queue.head(20).iterrows():
            lines.append(
                f"- {row['epic_id']}: action={row['stage_g_action']}; pred={row['gatevetter_prediction']}; "
                f"truth={row['manual_truth_class']}; score={row['gatevetter_score']}; "
                f"reason={row['gatevetter_v0_reason']}; trace={row['rule_trace']}"
            )
    ...
    lines.extend(
        [
            "",
            "Post-run assessment",
            f"- Beats AutoVet v2 blind: {'yes' if stage_g_recovered > 0 and len(false_promotions) < 7 else 'partial/no'} "
            f"(AutoVet v2 strict recall was 0/4 with 7 candidate-like promotions; GateVetter v0.1 strict recall is "
            f"{positive_recall_hits}/4, Stage G review recovery is {stage_g_recovered}/4, "
            f"false candidate promotions are {len(false_promotions)}, and false Stage G review promotions are {len(false_stage_g_promotions)}).",
            f"- Recovered manual candidates as candidate/caveated: {stage_g_recovered} / {len(positive_truth)}.",
            ...
            "- Gates too harsh: any remaining missed manual candidate has failed or missing diagnostics that do not match the narrow "
            "caveated survivor templates.",
            "- Gates too weak: caveated Stage G review must be monitored for uncertain_hold leakage; false Stage G review promotions are "
            "the key v0.1 risk metric.",
            "- Next threshold adjustments: tune OOT caveat upper bound, SNR upper cap, and huge-fallback odd/even cutoff on a larger "
            "blind set before allowing GateVetter to control non-manual Stage G queueing.",
        ]
    )
